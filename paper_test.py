...
def render(scored_rows):
    ...
    quotes = {}
    if opens:
        use_live = st.toggle("Use live prices (delayed ~15 min)", value=False)
        if use_live:
    ...
    with t2:
        ...
        if not opts:
            ...
            stop = stock["price"] - mult * stock["atr"]
            risk = shares * (stock["price"] - stop)

            st.markdown(
                f"<div class='pt-note'><b>{shares:,} shares at "
                f"₹{stock['price']:,.1f} = ₹{cost:,.0f}</b><br>"
                f"Exit level ₹{stop:,.1f} "
                f"({(1-stop/stock['price'])*100:.1f}% below). "
            ...
            if st.button("Place practice order", type="primary"):
                if len(reason.strip()) < 15:
                    ...
                    conn.execute(
                        ...
                        "nifty_at_buy) VALUES (?,?,?,?,?,?,?,?,?,?)",
                        (stock["ticker"], stock["name"], shares, stock["price"],
